...
export function RecommendationCard({ movie, media_type, children, showText }: MovieCardProps) {
  ...
  return (
    <div>
      <div onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} className="group relative rounded-xl bg-background-main p-2 transition-all duration-300">
        ...
        <div className="relative transform transition-transform duration-300 group-hover:scale-95">
          <Link href={`/${media_type ?? "movie"}/${movie.id}`}  className="block focus:outline-none">
            <div
              className="relative w-full pb-[56.25%] overflow-hidden rounded-xl bg-mediaCard-hoverBackground bg-cover bg-center transition-[border-radius] duration-300 group-hover:rounded-lg"
              style={{
                backgroundImage: movie.backdrop_path ? `url(https://image.tmdb.org/t/p/w1280${movie.backdrop_path})` : undefined,
